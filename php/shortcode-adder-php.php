    ...
    if($formType == 's') {
        ...
        if (isset($dbshortcodeval['simple_form'])) {
            if ($function_adder_simple == false) {
                ...
                $action= "{$domain_name}/api/xml/listsubscribe?authtoken={$apikeyval}&scope=CampaignsAPI&version=1&resfmt=JSON&listkey={$listkey}&";
                echo "<style>
                        * {
                            padding: 0px;
                            margin: 0px;
            ...
            // if($dbshortcodeval != '' && $dbshortcodeval['response_area_shower'] == 'simple_form_response' ) {
                $starting_part  = "<!--Start Zoho Campaigns Signup Form-->" . "<link rel='stylesheet' type='text/css' href='https://campaigns.zoho.com/css/zc_min_all.css'><div id='zc_simple_$sNoFn' name='zcampaignminiform'>";
